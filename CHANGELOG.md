...
- Scoped port: skipped upstream's `run_claude`/`run_cursor` native hook entry points, `process_claude_payload`, and the Copilot CLI `modifiedArgs`-preserving restructure — these are new host integrations, not part of this security fix, and `run_claude`/`run_cursor` have no dispatch wiring in this fork's `main.rs` (would be dead code). Also skipped upstream's `super::constants`/`crate::core::stream` module imports (cosmetic-only for this port; used inline literals and a direct `.stdin(Stdio::null())` instead). Redirect-bypass fix `e16aa26` still tracked separately — see token-diet issue artificemachine/token-diet#5.
