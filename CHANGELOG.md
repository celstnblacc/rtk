...
## [Unreleased] — celstnblacc/rtk fork sync (v0.34.7 to v0.43.0)

### Added

- **hook_cmd:** `run_claude()` now honours the token-diet `~/.config/token-diet/rtk-disabled` sentinel (extracted as pure `token_diet_rtk_disabled(home)` for testability) — the native `rtk hook claude` path this fork's installer wires up by default, superseding the legacy `hooks/claude/rtk-rewrite.sh` script as of upstream's migration to `CLAUDE_HOOK_COMMAND`.
- **hooks/claude/rtk-rewrite.sh:** same sentinel added to the legacy script as defense-in-depth for any host still on the pre-migration install path; bumped `rtk-hook-version` 3 → 4 and `CURRENT_HOOK_VERSION` to match.
- Verified telemetry stays opt-in by default on this upstream version (`TelemetryConfig::enabled` derives `Default` → `false`, `maybe_ping()` gates on it, pinned by upstream's own `test_telemetry_default_disabled`) — no fork override needed, matches this fork's prior telemetry-off policy.

### Fixed

- **tests:** added `.cargo/config.toml` with an `[env]` table and a `[target.'cfg(all())'] runner` override, all addressing the SAME root cause and pre-existing in upstream's own test suite, unrelated to this fork's changes:
  - **Root cause:** when `cargo test` runs from inside a git hook (e.g. this repo's own pre-commit hook), git has already set `GIT_DIR` / `GIT_WORK_TREE` / `GIT_INDEX_FILE` in the hook's environment, pointing at the *outer* repo being committed. Test code that shells out to `git -C <tempdir>` or `Command::new("git").current_dir(tempdir)` inherits those vars, and git's explicit `GIT_DIR` override takes precedence over `-C`/`current_dir()` — silently redirecting "isolated" git operations in `cmds::git::git`'s `*_propagates_failure` tests and `tests/guard_integration_test.rs` onto the outer repo's real index/config/refs instead of the tempdir fixture. Observed side effects: nondeterministic test failures, this checkout's real `core.bare` config getting set to `true`, and throwaway `git commit`s in `guard_integration_test.rs` triggering this machine's real global pre-commit hooks.
  - `runner = "env -u GIT_DIR -u GIT_WORK_TREE -u GIT_INDEX_FILE -u GIT_PREFIX --"` — the actual fix: strips the inherited vars at the point the compiled test binary is invoked, since `[env]` can only set variables, not unset inherited ones.
  - `RUST_TEST_THREADS = "1"` — belt-and-suspenders: also avoids any remaining `std::env::set_current_dir()`-based races in `src/hooks/init.rs` tests under parallel execution.
  - `GIT_CONFIG_GLOBAL = "/dev/null"` — belt-and-suspenders: keeps any still-real git operations in tests from touching this machine's global git config.
  - Verified: full suite (2241 lib tests + 5 integration binaries) passes clean under the exact `GIT_DIR`/`GIT_INDEX_FILE` pollution a hook-triggered run injects.

### Fixed (landing the sync — re-wiring fork-only features dropped by the tree merge)

- **doctor:** `rtk doctor` (fork commit `7b1f1b0`) was silently orphaned by the sync — `src/main.rs` and `src/cmds/system/mod.rs` exist upstream too, so merging in upstream's tree wholesale dropped the fork's `Doctor` CLI wiring while leaving `doctor.rs` itself in place, making its entire module dead code under this crate's `[lints] warnings = "deny"` gate. Re-added: `Commands::Doctor { json }` variant, its dispatch arm, its entry in `RTK_META_COMMANDS` (needed for `test_every_subcommand_is_classified`), its skip-hook-warning guard, and `Tracker::get_failure_rate_7d()` (same fate — added to `tracking.rs` by the same fork commit, wiped by the same wholesale checkout; rebuilt on top of the now-current `count_commands_since`/`parse_failures_since` helpers instead of raw SQL). 3 clippy `useless_use_of_vec` fixes in `doctor.rs`'s own tests (pre-existing, newer clippy than when `7b1f1b0` was authored).
- **filters:** removed `src/filters/mvn-build.toml` — upstream's `feat(mvn)!:` commit replaced it with the native `src/cmds/jvm/mvn_cmd.rs` module but the old TOML filter survived on this fork's `master` (never merged, no upstream equivalent commit to remove it), causing `test_builtin_filter_count`/`test_new_filter_discoverable_after_concat` to see 64 filters instead of the expected 63.
- **grep_cmd:** `#![allow(dead_code)]` — `src/cmds/system/grep_cmd.rs` is superseded by `cmds::system::search` (upstream's own doing; `main.rs` never imports `grep_cmd` on this tag) but not yet deleted upstream. Pre-existing in the exact v0.43.0 tag, unrelated to this fork; this crate's `warnings = "deny"` makes it a hard build failure rather than a lint, so it needed *some* resolution to make any progress syncing.
- Note: a broader pass of stale, upstream-renamed fork docs (`ARCHITECTURE.md` → `docs/contributing/ARCHITECTURE.md`, `AGENTS.md`/`GEMINI.md`/`SOUL.md`, old flat `.claude/skills/*.md` → `*/SKILL.md`, etc. — ~35 files) is deliberately deferred: none of them break a build or test, they're pure documentation staleness from the wholesale tree merge, and cleaning them up doesn't belong in a security/sync-focused pass.

<!--
Fork history (pre-sync): the 0.34.2-security.1 entry above is restored
in its original file position. The remaining fork-only entries
(0.34.3-security.1 through 0.34.7) are appended at the very end of
this file, after upstream's full history, to satisfy CHANGELOG's
append-only diff check while keeping all pre-sync content intact.
-->
...
- **lint:** silenced two `dead_code` false positives that broke `cargo clippy --all-targets -- -D warnings`. `FILTERS_TOML` and `TomlFilterRegistry::load` are live in the real binary (`load()` → the `REGISTRY` lazy_static → `find_matching_filter` → `main.rs`), but `--all-targets` also builds a test binary that never links main's call path, so the whole chain reads as dead there and `[lints.rust] warnings = "deny"` turned it into a hard error. Annotated with `#[cfg_attr(test, allow(dead_code))]` rather than a bare `#[allow]`, so genuinely dead code is still caught in real builds.
- **security:** cleared all three open RUSTSEC advisories. `crossbeam-epoch` 0.9.18 to 0.9.20 for RUSTSEC-2026-0204 (invalid pointer dereference in the `fmt::Pointer` impl for `Atomic`/`Shared`) — transitive via `crossbeam-deque` -> `ignore`, semver-compatible lockfile update. `quick-xml` 0.37 to 0.41 for RUSTSEC-2026-0195 (unbounded namespace-declaration allocation in `NsReader`, a memory-exhaustion DoS) and RUSTSEC-2026-0194 (quadratic run time when checking a start tag for duplicate attribute names). quick-xml is a direct dependency, and 0.41 is a breaking release, so two call sites were migrated: `BytesText::unescape()` is gone in favour of content-model-specific accessors, so `dotnet_cmd.rs` now uses `xml10_content()` (.csproj is XML 1.0); and `Attribute::decode_and_unescape_value` was replaced by the version-aware `decoded_and_normalized_value`, so `dotnet_trx.rs` passes `XmlVersion::Explicit1_0` (TRX files carry an explicit 1.0 declaration). Both are behaviour-preserving for the XML these paths parse. `cargo audit` now reports no vulnerabilities.
