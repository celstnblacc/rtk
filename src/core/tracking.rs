...
impl Tracker {
    ...
    /// token-diet integration: backs `rtk doctor`'s failure_rate check.
    pub fn get_failure_rate_7d(&self) -> Result<f64> {
        let cutoff = chrono::Utc::now() - chrono::Duration::days(7);
        let total_cmds = self.count_commands_since(cutoff)?;
        if total_cmds == 0 {
            return Err(anyhow::anyhow!("no command history"));
        }
        let failures = self.parse_failures_since(cutoff)?;
