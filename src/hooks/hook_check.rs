...
mod tests {
    ...
    #[test]
    fn test_hook_script_contains_token_diet_sentinel() {
        let script = include_str!("../../hooks/claude/rtk-rewrite.sh");
        assert!(
            script.contains(".config/token-diet") && script.contains("rtk-disabled"),
            "legacy rtk-rewrite.sh must honour the token-diet rtk-disabled sentinel"
        );
    }

    #[test]
    fn test_hook_version_constant_matches_script() {
        let script = include_str!("../../hooks/claude/rtk-rewrite.sh");
        assert_eq!(
            parse_hook_version(script),
            CURRENT_HOOK_VERSION,
            "rtk-hook-version comment in rtk-rewrite.sh must match CURRENT_HOOK_VERSION"
        );
    }
