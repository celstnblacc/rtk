...
```bash
rtk init -g
# → Installs hook to $HOME/.claude/hooks/rtk-rewrite.sh
# → Creates $HOME/.claude/RTK.md (10 lines, meta commands only)
# → Adds @RTK.md reference to $HOME/.claude/CLAUDE.md
# → Prompts: "Patch settings.json? [y/N]"
# → If yes: patches + creates backup ($HOME/.claude/settings.json.bak)
...
# Manually edit $HOME/.claude/settings.json
# Restart Claude Code
```
...
#   - Context: $HOME/.claude/RTK.md
#   - Reference: @RTK.md line from $HOME/.claude/CLAUDE.md
...
# Add to PATH if needed ($HOME/.bashrc or $HOME/.zshrc)
